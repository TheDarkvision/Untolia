...
public sealed class StatsMenu : Menu
{
    ...
    private float DrawRightContent(SpriteBatch sb, PartyMember sel, Rectangle rightPane, bool measureOnly, float scroll)
    {
        ...
        if (!measureOnly && IsLineVisible(col1Y, rightPane))
            DrawLabelValue(sb, "Force", sel.BaseStats.Force.ToString(), new Vector2(col1X, col1Y));
        col1Y += row;
        if (!measureOnly && IsLineVisible(col1Y, rightPane))
            DrawLabelValue(sb, "Focus", sel.BaseStats.Focus.ToString(), new Vector2(col1X, col1Y));
        col1Y += row;
        if (!measureOnly && IsLineVisible(col1Y, rightPane))
            DrawLabelValue(sb, "Resolve", sel.BaseStats.Resolve.ToString(), new Vector2(col1X, col1Y));
        col1Y += row;
        if (!measureOnly && IsLineVisible(col1Y, rightPane))
            DrawLabelValue(sb, "Agility", sel.BaseStats.Agility.ToString(), new Vector2(col1X, col1Y));
        col1Y += row * 1.2f;
        ...
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Weapon", sel.WeaponId ?? "-", new Vector2(col2X, infoY));
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Shield", sel.ShieldId ?? "-", new Vector2(col2X, infoY));
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Armor", sel.ArmorId ?? "-", new Vector2(col2X, infoY));
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Trinket", sel.TrinketId ?? "-", new Vector2(col2X, infoY));
        infoY += row * 1.2f;

        if (!measureOnly) DrawSectionHeader(sb, "Affinities", new Vector2(col2X, infoY), rightPane);
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Resist", sel.Resist.Count == 0 ? "-" : string.Join(", ", sel.Resist),
                new Vector2(col2X, infoY));
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Weak", sel.Weak.Count == 0 ? "-" : string.Join(", ", sel.Weak),
                new Vector2(col2X, infoY));
        infoY += row;
        if (!measureOnly && IsLineVisible(infoY, rightPane))
            DrawLabelValue(sb, "Immune", sel.Immune.Count == 0 ? "-" : string.Join(", ", sel.Immune),
                new Vector2(col2X, infoY));
        infoY += row * 1.2f;
        ...
        if (def?.Appearance != null)
        {
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Age", def.Appearance.Age ?? "-", new Vector2(x, bioY));
            bioY += row;
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Build", def.Appearance.Build ?? "-", new Vector2(x, bioY));
            bioY += row;
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Hair", def.Appearance.Hair ?? "-", new Vector2(x, bioY));
            bioY += row;
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Eyes", def.Appearance.Eyes ?? "-", new Vector2(x, bioY));
            bioY += row;
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Skin", def.Appearance.Skin ?? "-", new Vector2(x, bioY));
            bioY += row;
            if (!measureOnly && IsLineVisible(bioY, rightPane))
                DrawLabelValue(sb, "Attire", def.Appearance.Attire ?? "-", new Vector2(x, bioY));
            bioY += row * 1.2f;
        }
