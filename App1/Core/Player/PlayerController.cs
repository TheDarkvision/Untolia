...
public sealed class PlayerController
{
        ...
        int blockedH)
    {
        ...
        var pw = playerTexture?.Width > 0 ? playerTexture.Width : 16;
        var ph = playerTexture?.Height > 0 ? playerTexture.Height : 16;
        ...
        if (!RectBlocked(blocked, blockedW, blockedH, desiredRect)) position.X = desired.X;

        desiredRect = new Rectangle((int)(position.X - pw * 0.5f), (int)(desired.Y - ph * 0.5f), pw, ph);
        if (!RectBlocked(blocked, blockedW, blockedH, desiredRect)) position.Y = desired.Y;
    }
    ...
    private static bool RectBlocked(bool[] blocked, int w, int h, Rectangle rect)
    {
        ...
        var right = Math.Min(w - 1, rect.Right - 1);
        var top = Math.Max(0, rect.Top);
        var bottom = Math.Min(h - 1, rect.Bottom - 1);
